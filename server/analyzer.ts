...
export async function analyzeWebsite(url: string): Promise<SEOReport> {
  try {
    ...
    // PageSpeed Insights (would need API key in production)
    const performance = {
      mobile: Math.floor(Math.random() * 40) + 60, // Mock data for demo
      desktop: Math.floor(Math.random() * 30) + 70, // Mock data for demo
    };
